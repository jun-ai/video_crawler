...
| Token | Hex | 用途 |
|-------|-----|------|
| `--color-accent` | `#F59E0B` | 强调（重要 CTA、警示、热度） |
| `--color-accent-hover` | `#D97706` | hover 态 |
...
| `--color-warning` | `#F59E0B` | 警告（同 accent） |
| `--color-danger` | `#DC2626` | 错误 / 删除 |
| `--color-info` | `#2563EB` | 信息（同 brand） |
...
|-------|-------|------|
| `--radius-sm` | 8px | 小组件 |
| `--radius-md` | 12px | 按钮（Blue 也用 12px） |
| `--radius-lg` | 16px | 卡片 |
| `--radius-xl` | 24px | 大卡片 / 模态框 |
| `--radius-2xl` | 32px | Hero 区 |
| `--radius-full` | 9999px | 药丸标签（Blue pill tag） |
...
（新增 72 和 88，对应 Blue 的 section padding）
...
### CTA 按钮（核心按钮，借鉴 Blue 蓝色渐变）

**主要 CTA**（等同于 Blue "立即激活"）：
```css
background: linear-gradient(#60A5FA 0%, #3B82F6 100%);  /* Blue 渐变 */
color: #FFFFFF;
...
| Variant | 样式 |
|---------|------|
| primary | 蓝色渐变 `bg-gradient(#60A5FA→#3B82F6)` text-white |
